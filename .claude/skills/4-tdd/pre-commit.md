...
- [ ] Run these checks in parallel (one message, multiple Task/subagent
      ...
      2. `npm run lint` (ESLint, includes eslint-plugin-jsx-a11y)
      3. `npm test` (jest, jsdom, includes jest-axe on every component
         that has it)
...
- [ ] `CLAUDE.md`, `docs/reference/*.md`, and `README.md` reflect this
      ...
      design-direction/architecture reference docs if this issue added,
...
- [ ] Every new component with rendered markup has an RTL test covering
      its meaningfully distinct render states (loading, empty, error,
      success, expanded/collapsed, etc.), not just the default render,
      plus a jest-axe (`toHaveNoViolations()`) assertion
- [ ] SCSS module exists for every new component, co-located in the
