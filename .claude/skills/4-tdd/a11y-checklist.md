...
## Color and Contrast
- [ ] Text contrast ratio minimum 4.5:1 against background (verify with
      `src/utils/contrast-ratio.ts`'s `contrastRatio()`, or add a case to
      `src/styles/design-tokens.contrast.test.ts` — not by eye)
- [ ] Interactive element contrast minimum 3:1
- [ ] Information never conveyed by color alone

`contrast-ratio.ts` only knows flat hex pairs — rgba-based tokens
(`$ink-mid`, `$ink-faint`, `$rust-mid`, `$rust-pale`) can't feed it
directly. Those need the real-browser sweep instead
(`.claude/skills/a11y-sweep/SKILL.md`), which reads actually-painted
pixels.
