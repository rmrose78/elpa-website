...
- **Color contrast is effectively unchecked by jest-axe.** axe-core's own
  ...
  this is what catches contrast failures on rgba-based tokens (`$ink-mid`,
  `$ink-faint`, `$rust-mid`, `$rust-pale`) that
  `src/utils/contrast-ratio.ts`'s hex-math can't compute (it only knows
  flat hex color pairs you hand-enter).
...
Precondition: none — `npm run test:a11y` starts its own dev server on a
dedicated port (5180, not this repo's normal 5173) via Playwright's
`webServer` config, so it never collides with a `npm run dev` you already
have running.
...
1. **Run the automated suite**: `npm run test:a11y`. This is
   ...
   desktop (1440px) viewports, plus the mobile nav menu open and a
   building-detail modal open state — the same "one check per meaningfully
   distinct render state" rule this repo already applies to jest-axe.
...
4. **Optionally run Lighthouse**: `npm run lighthouse:a11y` (starts its own
   headless Chrome against `http://localhost:5173` — start `npm run dev`
   first if it isn't already running; this script doesn't manage its own
   server lifecycle the way the Playwright config does). Report the score
   and flag anything it surfaces that axe didn't (mainly tap-target size
   and viewport meta checks — most of its rule set overlaps axe-core, so
   don't re-report duplicates).
...
- [pass/fail] building detail modal open
